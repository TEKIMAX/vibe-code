...
interface AuthContextType {
  ...
  login: (provider: 'google' | 'github' | 'workos', redirectUrl?: string) => void;
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const [authProviders, setAuthProviders] = useState<{ google: boolean; github: boolean; workos: boolean; email: boolean; } | null>(null);
  ...
  const fetchAuthProviders = useCallback(async () => {
    try {
      ...
      if (response.success && response.data) {
        setAuthProviders({ workos: false, ...response.data.providers });
      ...
      setAuthProviders({ google: false, github: false, workos: false, email: true });
      setHasOAuth(false);
  ...
  // OAuth login method with redirect support
  const login = useCallback((provider: 'google' | 'github' | 'workos', redirectUrl?: string) => {
